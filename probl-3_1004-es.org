...
b) A corto plazo los hogares no pueden sustituir fácilmente el gas
   natural y la reducción en el consumo es pequeña.  Pero a largo
   plazo, los hogares pueden reducir mucho más el consumo de gas
   utilizando otras fuentes de energía más baratas, mejorando el
   aislamiento térmico de las viviendas, instalando una caldera más
   eficiente, etc.
...
El segundo siempre gasta lo mismo: $P Q^d = \money{20}$, por lo que su
curva de demanda es $Q^d = 20 / P$. La pendiente de la curva de demanda
es:
#+begin_export latex
\[
  \frac{\diff Q^d}{\diff P} = - \frac{20}{P^2} = - \frac{Q^d}{P}
\]
#+end_export
Por tanto, la elasticidad de la demanda del segundo conductor es
unitaria:
#+begin_export latex
\[
  E_d = \frac{\diff Q^d}{\diff P}\frac{P}{Q_d} = - 1
\]
#+end_export
...
c) Ingresos:
   ...
   \price{5}$. A ese precio $Q^* = 3000$ conductores cruzan el túnel
   diariamente y la empresa obtiene unos ingresos de $I^* =
...
a) La demanda es igual al aforo de la sala cuando $P =
   \money{18.75}$. La elasticidad sería $E_d = -160 \cdot 18.75 / 5000
   = -0.6$: tramo inelástico de la demanda.


b) Los ingresos se maximizan cuando $E_d = -1$, lo que sucede cuando
   $P = \money{25}$ y se venden \num{4000} entradas.
