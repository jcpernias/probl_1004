...
La minimización de los costes requiere la igualación de la \RMST con
el precio relativo de los factores. Sin embargo, en esta empresa no se
cumple esa condición, ya que:
#+begin_export latex
 \[ \RMST = \frac{\PMgL}{\PMgK} = 3 < \frac{w}{r} = 4 \]
#+end_export
El valor de la \RMST indica que se podría producir la misma cantidad
de anillos si se alquilaran 3 impresoras más y se contratara un
trabajador menos. Esta opción reduciría los costes de producción
porque el salario que se ahorraría la empresa es mayor que el alquiler
de 3 impresoras.
...
a) La minimización de los costes requiere la igualación de la \RMST y
   el precio relativo de los factores:
   #+begin_export latex
   \[ \RMST  = 2 \frac{K}{L} = \frac{w}{r} = 2 \]
   #+end_export
   Esta condición de tangencia puede expresarse de forma más sencilla
   como: $K = L$.  Sustituyendo esta expresión en la función de
   producción y operando se llega a la demanda condicional de trabajo:
   $L(Q) = Q^{1/3}$ y la demanda condicional de capital: $K(Q) =
   Q^{1/3}$.

b) El coste total son las retribuciones de los factores de producción:
   $\CT = 16 L + 8 K$.  Sustituyendo en esta expresión las demandas
   condicionales de trabajo y capital y operando se obtiene la curva
   de costes a largo plazo: $\CT(Q) = 24 Q^{1/3}$. Producir
   $\num{8000}$ unidades costaría:
   #+begin_export latex
   \[ \CT(\num{8000}) = 24 \times \num{8000}^{1/3} = \money{480} \]
   #+end_export

   Si dividimos los costes totales por la producción obtenemos la
   curva de costes medios: $\CMe(Q) = 24 Q^{-2/3}$.

   Los costes marginales se obtienen derivando los costes totales:
   $\CMg(Q) = 8 Q^{-2/3}$.
...
La curva de coste medio es:
#+begin_export latex
\[
  \CMe(Q) = \frac{\CT}{Q} = Q^2 - 8 Q + 22
\]
#+end_export
El coste marginal es:
#+begin_export latex
\[
  \CMg(Q) = \frac{\diff \CT}{\diff Q} = 3 Q^2 - 16 Q + 22
\]
#+end_export
...
\]
#+end_export
condición que se cumple si $Q = 4$ unidades.
...
a) La función de producto total es: $Q = 3 L$. Invirtiendo esta
   expresión se obtiene la demanda condicional de trabajo a corto
   plazo: $L = Q/3$. La curva de costes a corto plazo es:
   $\CT_{\CP}(Q) = 3 + Q / 3$.
...
b) Igualando la $\RMST = K/L$ con el precio relativo de los factores,
   ...
   $K = L$.  Sustituyendo esta expresión en la función de producción y
   operando se obtienen las demandas condicionales de los factores a
   largo plazo: $L(Q) = Q^{1/2}$ y $K(Q) = Q^{1/2}$.  Finalmente, la
   curva de costes a largo plazo es: $\CT_{\LP}(Q) = 2 Q^{1/2}$.
...
c) En general, los costes a corto plazo son mayores que los costes a
   ...
   \]
   #+end_export
   Elevando los dos lados de la ecuación anterior al cuadrado y
   ...
   \]
   #+end_export
   Las dos raíces de la ecuación anterior coinciden: $Q_a =
   9$ unidades.
   ...
   Para cualquier otro nivel de producción los costes a corto plazo son
   mayores que los costes a largo plazo.
   #+xxx: endcol

   Sólo cuando $Q = 9$, coinciden el capital a corto plazo y el nivel
   óptimo a largo plazo: $K(9) = 9^{1/2} = 3$. Cuando $Q \neq 9$, a
   corto plazo la empresa no puede modificar $K$ y para ajustar su
   produccción no puede sustituir un factor por otro; tan sólo puede
   modificar $L$.
...
a) Las combinaciones de factores que se elegirían a corto plazo se
   encuentran en las intersecciones de la senda de expansión del
   producto, $\Kbar = 4$, con las respectivas isocuantas:
...
b) A largo plazo, la empresa seleccionaría aquellas cestas donde la
   ...
   cortan en una combinación que pertenece a la isocuanta $Q = 160$.
...
d) Incorrecta: el coste medio es creciente siempre que el coste
   marginal es /mayor/ que el coste medio.
...
a) Si $L= 16$ y $K = 4$, la producción máxima que puede obtener la
   empresa es: $Q = 10 \times 16^{1/2} 4^{1/2} = 80$. Por tanto, la
...
c) La condición de tangencia puede escribirse como: $L = 4 K$.
   ...
   ahora se contratan $4$ unidades más de trabajo y $1$ unidad más de
   capital.  El incremento del coste es:
   #+begin_export latex
   \[
     \incr C = w \incr L + r \incr K =
     20 \times 4 + 80 \times 1 = \money{160}
   \]
   #+end_export
