...
#+begin_export latex
 \[ r = 4 w  \]
#+end_export
Por otro lado, los costes son:
#+begin_export latex
 \[ \num{1152} = 144 w + 12 r \]
#+end_export
Resolviendo las dos últimas ecuaciones, se obtiene el valor del
salario, $w = \wage{6}$, y del precio del capital, $r = \price{24}$.
...
a) La minimización de los costes requiere la igualación de la \RMST y
   ...
   \[ Q = L^3 \]
   #+end_export
   Despejando $L$ de la expresión anterior, se llega a la demanda
   condicional de trabajo:
   #+begin_export latex
   \[ L(Q) = Q^{1/3} \]
   #+end_export
   Sustituyendo esta última expresión en la condición de tangencia,
   obtenemos la demanda condicional de capital:
   #+begin_export latex
   \[ K(Q) = Q^{1/3} \]
   #+end_export
...
b) El coste total son las retribuciones de los factores de producción:
   ...
   \[ \CT = 16 L + 8 K  \]
   #+end_export
   Sustituyendo en esta expresión las demandas condicionales de
   trabajo y capital y operando se obtiene la curva de costes a largo
   plazo:
   #+begin_export latex
   \[ \CT(Q) = 24 Q^{1/3}  \]
   #+end_export
   ...
   \[ \CMe(Q) = 24 Q^{-2/3}  \]
   #+end_export
   Calculando la derivada de los costes totales con respecto de la
   producción se obtiene la curva de costes marginales:
   #+begin_export latex
   \[ \CMg(Q) = 8 Q^{-2/3}  \]
   #+end_export
...
\]
#+end_export
por tanto, el coste medio alcanza un mínimo cuando la producción es $Q
= 4$ unidades.
