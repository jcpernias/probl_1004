...
\end{gather*}
#+end_export
Por tanto, si $\PMgL = \PMeL$ implica que $\CMg = \CVMe$. En ese caso,
se maximiza el producto medio y se minimiza el coste variable
medio. Como el coste medio es $\CMe = \CFMe + \CVMe$, concluimos que,
cuando se igualan el producto medio y el marginal, se cumple $\CMe >
\CVMe = \CMg$ y, finalmente, el coste medio es mayor que el marginal .
...
a) La minimización de los costes requiere la igualación de la \RMST y
   ...
   #+end_export
   Esta condición puede expresarse de forma más sencilla como: $K =
   L$.  Sustituyendo esta expresión en la función de producción y
   operando se llega a la demanda condicional de trabajo: $L(Q) =
   Q^{1/3}$ y a la demanda condicional de capital: $K(Q) = Q^{1/3}$.
...
c) En general, los costes a corto plazo son mayores que los costes a
   ...
   empresa no puede contratar en el corto plazo las unidades de
   capital que serían necesarias para minimizar los costes de
   producción.
...
a) Las combinaciones de factores que se elegirían a corto plazo se
   ...
   producto a corto plazo, $\Kbar = 4$, con las respectivas
   isocuantas:
