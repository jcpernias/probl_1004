...
c) El precio de equilibrio es inferior al precio mínimo y hay un
...
#+begin_export latex
\[
  \incrpct{P} = -6/300 \times 100 = -2
\]
#+end_export
Podemos determinar la variación de la cantidad demandada y de la
cantidad ofrecida usando los valores de las elasticidades de la
demanda y de la oferta, respectivamente:
#+begin_export latex
\begin{gather*}
  \incrpct{Q^{d}} = E_d \incrpct{P} = -1.5 \times (-2) = 3 \\
  \incrpct{Q^{o}} = E_o \incrpct{P} = 2 \times (-2) = -4
\end{gather*}
#+end_export
...
a) La curva de demanda es:
   #+begin_export latex
   \[
     X^d_a = 200 - 5 P_X + 3 \times 5 + 2 \times 100 = 415 - 5 P_X
   \]
   #+end_export
   Igualando oferta y demanda se obtiene el equilibrio: $P^*_X =
   \price{45}$ y $X^* = \units{190}$. Como el precio máximo es
   superior al precio de equilibrio, nada impide que este mercado
...
b) La nueva curva de demanda es:
   #+begin_export latex
   \[
     X^d_b = 200 - 5 P_X + 3 \times 5 + 2 \times 170 = 555 - 5 P_X
   \]
   #+end_export
   El nuevo precio de equilibrio sería $P_X^* = \price{65}$. Pero este
   precio no se puede alcanzar ya que es mayor que el precio
   máximo. El precio de mercado sería $P_X = \price{50}$, se
   produciría un exceso de demanda y se venderían todas las unidades
   ofrecidas: $X^o(50) = \units{200}$.
...
Un precio máximo mayor que el de equilibrio no impide que el mercado
se equilibre. En ese caso la cantidad vendida la determinaría la
intersección de las curvas de demanda y de oferta, por lo que la
