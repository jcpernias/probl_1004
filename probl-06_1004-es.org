...
b)
   ...
   #+xxx: col
   #+latex: \vspace{1ex}
   Todas las cestas indiferentes a $A$ cumplen la ecuación: $20 = XY$. La curva
...
c) La relación marginal de sustitución muestra a cuántas unidades de $Y$
   renunciaría Carlos a cambio de una unidad adicional de $X$.
   #+begin_export latex
   \[
     \RMS = \frac{\UMX}{\UMY} = \frac{Y}{X}
   \]
   #+end_export
   En la cesta $A$: $\RMS(2, 10) = 10 / 2 = 5$ unidades de $Y$ por unidad de
   $X$.
