...
Sabiendo que el precio del bien $X$ es $P_X = \price{10}$ y que el
precio del bien $Y$ es $\price{20}$ represente gráficamente las curvas
de Engel de ambos bienes. ¿Qué tipo de bienes son $X$ e $Y$?
...
cuando el el precio del bien $Y$ es $P_Y = \price{2}$ y la renta es $M
= \money{120}$:

#+xxx: fig fig-probl-6_1004-cpc22-*.pdf

a) ¿Cuántas unidades de $X$ compraría el consumidor si el precio de
   este bien es $P_X = \price{1}$?.

b) Obtenga también la cantidad óptima de $X$ cuando $P_X$ toma los
   valores $2, 3, 4$, y \price{5} y represente la curva de demanda de
   $X$.
